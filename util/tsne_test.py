...
if __name__=='__main__':
    ...
    #model = create_model(opt)
    choose_num=500
    styles=torch.zeros(choose_num*4,256,3,3)
    with torch.no_grad():
        for i, data in enumerate(dataset):
            ...
            if i==0:
                ...
                cv.imwrite('./TSNE/img2.png',person_gen[:,:,[2,1,0]])
            temp_0=model.style_kernels[0][0]
            temp_3=model.style_kernels[0][3]
            temp_5=model.style_kernels[0][5]
            temp_dtd=model.style_kernels[0][9]
            styles[i]=temp_0
            styles[i+choose_num*1]=temp_3
            styles[i+choose_num*2]=temp_5
            styles[i+choose_num*3]=temp_dtd
    x=styles.reshape(choose_num*4,-1)
    result=tsne.fit_transform(x)
    result[choose_num*3:]=remove_outliers(result[choose_num*3:],100)
    ...
    #plt.legend()
    #plt.scatter(result[choose_num*3:,0], result[choose_num*3:,1],c='c',marker='o')#dtd
    #plt.legend()
    plt.show()
    plt.savefig("./TSNE/stylemix_pca_wdtd_removedtd"+str(choose_num)+".png")