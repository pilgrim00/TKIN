...
LIMB_SEQ_ = [[1,2], [1,5], [2,3], [3,4], [5,6], [6,7], [2,8], [8,9],
           [9,10], [5,11], [11,12], [12,13], [1,0], [0,14], [14,16],
           [0,15], [15,17], [1,16], [1,17],[8,11]]
COLORS = [[255, 0, 0], [255, 85, 0], [255, 170, 0], [255, 255, 0], [170, 255, 0], [85, 255, 0], [0, 255, 0],
          [0, 255, 85], [0, 255, 170], [0, 255, 255], [0, 170, 255], [0, 85, 255], [0, 0, 255], [85, 0, 255],
          [170, 0, 255], [255, 0, 255], [255, 0, 170], [255, 0, 85]]
COLORS_ = [[255, 0, 0], [255, 85, 0], [255, 170, 0], [255, 255, 0], [170, 255, 0], [85, 255, 0], [0, 255, 0],
          [0, 255, 85], [0, 255, 170], [0, 255, 255], [0, 170, 255], [0, 85, 255], [0, 0, 255], [85, 0, 255],
          [170, 0, 255], [255, 0, 255], [255, 0, 170], [255, 0, 85],[255,100,0],[100,255,0]]
...
def draw_pose_from_cords(pose_joints, img_size, radius=2, draw_joints=True):
    ...
    return colors, mask
def new_draw_pose_from_cords(pose_joints, img_size,draw_joints=True):
    limb=LIMB_SEQ_
    #colors = np.zeros(shape=img_size + (3, ), dtype=np.uint8)
    mask = np.zeros(shape=img_size + (3, ), dtype=np.uint8)
    limbs = np.zeros(shape=(len(limb),256,256), dtype=np.uint8)
    stickwidth = 4
    if draw_joints:
        i=0
        for f, t in limb:
            temp = np.zeros(shape=img_size, dtype=np.uint8)
            #canvas = colors.copy()
            from_missing = pose_joints[f][0] == MISSING_VALUE or pose_joints[f][1] == MISSING_VALUE
            to_missing = pose_joints[t][0] == MISSING_VALUE or pose_joints[t][1] == MISSING_VALUE
            if from_missing or to_missing:
                continue
            X=[pose_joints[f][0],pose_joints[t][0]]
            Y=[pose_joints[f][1],pose_joints[t][1]]
            mX = np.mean(X)
            mY = np.mean(Y)
            length = ((X[0] - X[1]) ** 2 + (Y[0] - Y[1]) ** 2) ** 0.5
            angle = math.degrees(math.atan2(X[0] - X[1], Y[0] - Y[1]))
            polygon = cv2.ellipse2Poly((int(mY), int(mX)), (int(length / 2), stickwidth), int(angle), 0, 360, 1)
            out=cv2.fillConvexPoly(temp, polygon,1)
            mask+=out.reshape(256,256,1)
            limbs[i]=out
        i+=1
    return  mask,limbs
...
if __name__ == "__main__":
    ...
    #import os
    i = 5
    data_root='/apdcephfs/share_1227775/BaseAI/workspace/jiaxianchen/inshop/PISE/'
    df = pd.read_csv(data_root+'fasion-annotation-train.csv', sep=':')
    for index, row in df.sample(frac=1).iterrows():
        pose_cords = load_pose_cords_from_strings(row['keypoints_y'], row['keypoints_x'])

        mask,bone = new_draw_pose_from_cords(pose_cords, (256,256))
        keypoints=cords_to_map(pose_cords, (256,256))
        print(mask.shape)
        print(bone.shape)
        print(keypoints.shape)
        keypoints=np.sum(keypoints,axis=2)
        print(keypoints.shape)
        cv2.imwrite('./mask_1.png',(mask+1)/2*255)
        cv2.imwrite('./mask_2.png',(keypoints+1)/2*255)
        #cv2.imwrite('./mask2_1.png',bone[0]*255)
        exit()