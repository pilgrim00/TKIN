...
if __name__=='__main__':
    ...
    #model = create_model(opt)
    choose_num=500
    styles=torch.zeros(choose_num*3,256)
    with torch.no_grad():
        for i, data in enumerate(dataset):
            ...
            #cv.imwrite('./TSNE/img.png',person_gen[:,:,[2,1,0]])
            #exit()
            #print(model.net_G.codes_vector.shape)
            temp_0=model.net_G.codes_vector[0][0]
            temp_3=model.net_G.codes_vector[0][3]
            temp_5=model.net_G.codes_vector[0][5]
            styles[i]=temp_0
    ...
    #plt.legend()
    plt.show()
    plt.savefig("./TSNE/style_pise_mix_nos1_pca"+str(choose_num)+".png")