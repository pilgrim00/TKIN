...
def feats_norm(feats):
    #feats[1,7,h,w]
    _,c,_,_=feats.shape
    for i in range(c):
        feats_min=torch.min(feats[0][i])
        feats_max=torch.max(feats[0][i])
        if feats_max-feats_min<1e-1:
            range_=1
        else:
            range_=feats_max-feats_min

        feats[0][i]=(feats[0][i]-feats_min)/(range_)
    return feats
...
class Boneline(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.net_G = network.define_g(opt,filename='generator_boneline',image_nc=opt.image_nc, structure_nc=opt.structure_nc, ngf=64,
    ...
    def set_input(self, input):
        ...
        input_P2, input_BP2, input_SPL2, label_P2,label_P1 = input['P2'], input['BP2'], input['SPL2'], input['label_P2'],input['label_P1']

        if len(self.gpu_ids) > 0 and self.opt.dist==0:
            self.input_P1 = input_P1.cuda(self.gpu_ids[0])#相比async,non_blocking should be used instead.
            ...
            self.label_P1 = label_P1.cuda(self.gpu_ids[0])
        if self.opt.dist==1:
    ...
    def test(self):
        ...
        #self.par2_=feats_norm(self.par2_)
        ...
        #    w.add_graph(self.net_G,(self.input_P1, self.input_P2,self.input_BP1, self.input_BP2, self.input_SPL1, self.input_SPL2))
        #exit()
    ...
    def get_pic(self):
        ...
        self.input_BP2[self.input_BP2>1.0]=1.0
        self.input_BP1[self.input_BP1>1.0]=1.0
        result1 = torch.cat([self.input_BP1, self.input_BP2], 3)
        self.save_results(result1, data_name='all_1')
        result2 = torch.cat([self.input_SPL1,self.input_SPL2], 3)
        self.save_results(result2, data_name='all_2')
        #exit()        
    def val(self):
        """Forward function used in test time"""
        self.img_gen, self.loss_reg,self.par2_,self.par1_= self.net_G(self.input_P1, self.input_P2,
        ...
        #print(torch.min(self.input_BP2),torch.max(self.inp))
