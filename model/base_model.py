...
class BaseModel():
    ...
    def setup(self, opt):
        ...
        if not self.isTrain or opt.continue_train:
            #写一个实现自动检测是否存在目标文件，再进行加载的代码。
            for file in sorted(os.listdir(opt.checkpoints_dir+'/'+str(opt.name))):
                if 'latest' in file:    
                    #print('model resumed from %s iteration'%opt.which_iter)
                    name=file.split('_net')[0]
            if opt.which_iter:
                print('model resumed from %s iteration'%opt.which_iter)
                self.load_networks(opt.which_iter)
            if not opt.which_iter and  name is not None:
                print('model resumed from %s iteration'%name)
    ...
    def load_networks(self, which_epoch):
        ...
        for name in self.model_names:#['G','D']
            if isinstance(name, str):
