...
class PoseGenerator(BaseNetwork):
    ...
    def code_reg(self,code1,exits1,code2,exists2):
        ...
        #mu1=code1.mean(dim=2)#均值
        #mu2=code2.mean(dim=2)
        #std1=code1.std(dim=2)#标准差
        #std2=code2.std(dim=2)
        #print(mu1.mean())
        #loss=torch.log(std2/(std1+eps)+eps)+(std1**2+(mu1-mu2)**2)/(2*std2**2+eps)-0.5
        loss=F.mse_loss(code1,code2)
        #print(loss)
        #loss+=F.mse_loss(code1[:,8,:],code2[:,8,:])
        return loss
    def forward(self, img1, img2, pose1, pose2, par1, par2,dtd):
        ...
        loss_reg = F.mse_loss(dtd_feat, dtd_code)*0.2
        dtd_feat=self.res_dtd(dtd_feat)
        dtd_img=self.dec_dtd(dtd_feat)
        #
        parcode2 = self.efb(parcode2, par2, codes_vector1, exist_vector1)
        ...
        parcode1 = self.efb(parcode1, par1, codes_vector1, exist_vector1)
