...
class Bonelinethre(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.net_G = network.define_g(opt,filename='generator_bonelinethre',image_nc=opt.image_nc, structure_nc=opt.structure_nc, ngf=64,
                                use_spect=opt.use_spect_g, norm='instance', activation='LeakyReLU',use_sn=opt.sn,gt_par=opt.gt_par,
                                use_rec=opt.use_rec,ker_size=opt.ker_size,patch_dec=opt.patch_dec,use_bank=opt.use_bank).to(device)
    ...
    def test(self):
        ...
        #self.par_metric.add(self.par2_[:,1:,:,:],self.input_SPL2[:,1:,:,:])
        #print(torch.max(self.par2_[0][5]))
        ...
        #self.save_results(self.par2_, data_name='par_out')
    ...
    def backward_G(self):
        ...
        self.loss_par1 = self.L1loss(self.par2_, self.input_SPL2)*5+self.L1loss(self.par1_, self.input_SPL1)*5
